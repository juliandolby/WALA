...
public class LoopHelper {
  ...
  public static LoopType getLoopType(PrunedCFG<SSAInstruction, ISSABasicBlock> cfg, Loop loop) {
    // TODO: add unsopported loop type
    ...
    // If loopControl successor is loopHeader then it's a do loop, no matter they are the same block or different
  ...
  /**
   * Find out if the given chunk is in the loop and before the conditional branch of the loop control
   * 
   * @param cfg The control flow graph
   * @param chunk The instructions to be used to check
   * @param loops All the loops that's in the control flow graph
   * @return True if the given chunk is in the loop and before the conditional branch of the loop control
   */
